...
function readShareCode() {
  ...
  return new Promise(async resolve => {
   $.get({url: `http://cdn.boledao.com/shareCodes/cash.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function getAuthorShareCode(url = "http://cdn.boledao.com/shareCodes/jd_updateCash.json") {
  return new Promise(resolve => {
    $.get({url, headers:{
...
function getAuthorShareCode2(url = "http://cdn.boledao.com/shareCodes/jd_updateCash.json") {
  return new Promise(resolve => {
    $.get({url, headers:{
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
      }, timeout: 200000,}, async (err, resp, data) => {
