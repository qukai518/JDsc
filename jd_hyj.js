...
function submitCode(shareCode, type) {
    ...
    $.get({url: `htt//w.helpu.cf/jddes/submit.php?code=${shareCode}&type=${type}&user=${$.UserName}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function readShareCode(type) {
  return new Promise(async resolve => {
    $.get({url: `http://www.helpudes/getcode.php?type=${type}&num=20`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
