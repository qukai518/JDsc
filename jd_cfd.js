...
!(async () => {
  ...
  if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/inoyna12/updateTeam@master/shareCodes/cfd.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/inoyna12/updateTeam@master/shareCodes/cfd.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://trasfer.nz.lu/cfd`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
