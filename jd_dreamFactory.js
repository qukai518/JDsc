...
$.tuanIds = ['T_zZaWP6by9yA1wehxM4mg=='];
$.appId = 10001;
if ($.isNode()) {
...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN(), res2 = await updateTuanIdsCDN("http://cdn.boledao.com/shareCodes/jd_updateFactoryTuanId.json")
  if (!res) res = await updateTuanIdsCDN('http://cdn.boledao.com/shareCodes/jd_updateFactoryTuanId.json');
...
function updateTuanIdsCDN(url = 'http://cdn.boledao.com/shareCodes/jd_updateFactoryTuanId.json') {
  return new Promise(async resolve => {
    const options = {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: "http://cdn.boledao.com/shareCodes/jxfactory.json",headers:{
      "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
    }}, async (err, resp, data) => {
      try {
        if (err) {
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if ($.shareCodesArr[$.index - 1]) {
      ...
      $.newShareCodes = [...inviteCodes, "p15jzNJ363IRRYrz5uiijQ==","kMVrN_EU0rOG1TiQJoSHjg=="];
    }
  ...
  return new Promise(async resolve => {
    ...
    if (!tuanActiveId) {
      ...
      if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
        ...
        if (!$.tuanConfigs) {
          await updateTuanIdsCDN('http://cdn.boledao.com/shareCodes/jd_updateFactoryTuanId.json');
          if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
            tuanActiveId = $.tuanConfigs['tuanActiveId'];
            console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
          } else {
