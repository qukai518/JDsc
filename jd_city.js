...
!(async () => {
  ...
  readShareCodeRes = await getAuthorShareCode('https://raw.githubusercontent.com/inoyna12/updateTeam/master/shareCodes/city.json')
  if (!readShareCodeRes) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/inoyna12/updateTeam@master/shareCodes/city.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    readShareCodeRes = await getAuthorShareCode('https://inoyna12.coding.net/p/updateteam/d/updateTeam/git/raw/master/shareCodes/city.json')
  }
  await requireConfig();
  if (exchangeFlag) {
    console.log(`脚本自动抽奖`)
