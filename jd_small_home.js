...
[task_local]
#东东小窝
16 6,23 * * * jd_small_home.js, tag=东东小窝, img-url=https://raw.githubusercontent.com/58xinian/icon/master/ddxw.png, enabled=true

================Loon==============
[Script]
cron "16 6,23 * * *" script-path=jd_small_home.js, tag=东东小窝

===============Surge=================
东东小窝 = type=cron,cronexp="16 6,23 * * *",wake-system=1,timeout=3600,script-path=jd_small_home.js

============小火箭=========
东东小窝 = type=cron,script-path=jd_small_home.js, cronexpr="16 6,23 * * *", timeout=3600, enable=true
 */
const $ = new Env('东东小窝');
...
!(async () => {
  ...
  await updateInviteCodeCDN('https://raw.githubusercontent.com/inoyna12/updateTeam/master/shareCodes/jd_updateSmallHomeInviteCode.json');
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.token = $.helpToken[i];
