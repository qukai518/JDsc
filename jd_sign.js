...
$.get = validator.injectToRequest2($.get.bind($), 'channelSign')
$.post = validator.injectToRequest2($.post.bind($), 'channelSign')
...
function taskUrl(turnTableId) {
  const url = `https://jdjoy.jd.com/api/turncard/channel/detail?turnTableId=${turnTableId}&invokeKey=ztmFUCxcPMNyUq0P`
  return {
    url,
    headers: {
...
function tasPostkUrl(turnTableId) {
  const url = `https://jdjoy.jd.com/api/turncard/channel/sign?turnTableId=${turnTableId}&fp=${fp}&eid=${eid}&invokeKey=ztmFUCxcPMNyUq0P`
  return {
    url,
    headers: {
      "Accept": "application/json, text/plain, */*",
